...
In this post, I will assume that you have a little bit of math knowledge and that you can read mathematical symbols. I also assume that you know about the compute pipeline and know a bit about how compute shaders work. 
...
where $n$ is the amount of neighbouring non-solid cells and $\Delta t$ is the time-step since the last simulation frame. For any cell that is solid the pressure and velocity can be set to 0.
...
the pressure needs quite a lot of iterations to settle down. There is no way to run this an infinite amount of times, so for each pass of the solver the pressure is only updated a fixed amount of times (in my case 16).  
...
What yoy can also notice from the simulation is that there forms a natural vortex pattern in the velocities. This effect will cause the smoke to spread out and create those curls that are usually associated with smoke.
...
This brings one major problem, namely that it is possible but very computationally difficult to distribute the velocity. But since this a sequence we can instead can just move one step backwards and put that velocity at the position on the chosen face

$$
q^{n+1}_p=q^n_{p-\Delta t\vec{u}^n_p}
$$

This is computationally very simple, since that only takes two samples of the velocity. With this method, the second sample only needs to be the component of the face that is being computed.
...
Each step of the raymarcher, the checked point is shifter over some step size. Then I sample the concentration buffer to know how much if any smoke is at that point. All the concentration values are summed up and the sum is used to determine the final opacity and color of the pixel using the formula [^6]:
...
| Buffer ⟶ Texture | 11.13ms | <span style="color: #a6d189ff">-10.4%</span> | <span style="color: #a6d189ff">-32.3%</span> |
